...
|---|---|---|---|---|---|---|
| 2 | Phone | 1.1 | [`cf2c0fc`](https://github.com/SailorDave17/race-timer/commit/cf2c0fc) | `internal` | 2026-09-26 | First phone build, and the first release cut by CI: tag `v1.1`, [run 36221704853](https://github.com/SailorDave17/race-timer/actions/runs/36221704853), both certificates verified on the runner, committed as `completed`. The empty "Untitled release" draft that had sat on this track was replaced by the publish. Console, 2026-09-26: track **Active**, release *"Available to internal testers"*, tester list *Watch Testers* (7). |
| 3 | Wear | 1.1 | [`cf2c0fc`](https://github.com/SailorDave17/race-timer/commit/cf2c0fc) | `wear:internal` | 2026-09-26 | Same tag and run as row 2, uploaded a minute earlier. Console, 2026-09-26: release *"Available to internal testers"*, but the **track reads Inactive**. The Wear OS form factor was at 1 of 3: no Wear screenshots on the store listing, and not opted in. The same day, screenshots 01–08 were attached and the opt-in accepted. Both are saved as changes *not yet sent for review*, and the Console will not send them until a closed-testing release exists. Until that review completes, treat this build as **not reaching watches**. |
| 1 | Wear | 1.0 | [`089f216`](https://github.com/SailorDave17/race-timer/commit/089f216) | `wear:internal` | 2026-08-13 | First build. Track cell read *"Internal testing"* — the Console's label — until 2026-08-18, when the Play API was asked directly and answered **`wear:internal`**, a Wear-form-factor track distinct from the plain `internal` track, which is empty. The label and the track id are not the same string, and #81's workflow publishes by id. Commit **re-taken a second time**: prepared from `cadaea9` on `develop`, then rebuilt from `release` at `089f216`, which is the artifact Play accepted. **Rolled out to internal testers** — owner-asserted 2026-08-17. This cell read *"sitting as a draft on the track — no tester has it yet"* until then: written true at upload time on 2026-08-13, and never revisited. **Re-read 2026-09-26, when rows 2 and 3 were written**: the Console showed this release *"Available to internal testers"* on a `wear:internal` track reading **Inactive**, and the app's installed audience at **0**. So the 2026-08-17 rollout reached the track, and whether it reached any watch is doubtful (see row 3). |
