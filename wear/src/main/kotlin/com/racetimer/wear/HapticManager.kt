...
class HapticManager(context: Context) {
     ...
     * `VibrationAttributes` *itself* is API 30, so the field below is safe to build at any level this
     * app runs on; it is only the call that is gated.
     *
    ...
    // Built once. Constructing a VibrationAttributes is cheap, but a cue is issued on a deadline and
    // allocation on that path is the kind of thing that ends up in a timing investigation later.
